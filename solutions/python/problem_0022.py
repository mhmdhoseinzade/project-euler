...
class Solution(ISolution):
    ...
    def answer_one():
        Solution.NAMES.sort()
        s = 0
        for index, name in enumerate(Solution.NAMES, start=1):
            for letter in name:
                s += (ord(letter) - 64) * index
        return str(s)
    ...
    def answer_two():
        ...
        Solution.NAMES.sort()
        result = reduce(lambda x,y: x + y,[index * (ord(letter) - 64 ) for index, name in enumerate(Solution.NAMES, start=1) for letter in name])
        return str(result)        
